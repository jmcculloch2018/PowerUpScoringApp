...
class RefereeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
		AppDelegate.manager.defaultSocket.on("updateAutoCrosses") { data, ack in
		...
		AppDelegate.manager.defaultSocket.on("updateClimbs") { data, ack in
			let isRed = data[0] as! Bool;
			if (isRed) {
		...
		AppDelegate.manager.defaultSocket.on("updateParks") { data, ack in
			let isRed = data[0] as! Bool;
			if (isRed) {
				let redParks = data[1] as! Int;
				self.redParksLabel?.text = "# Climbs: \(redParks)";
				self.redParksStepper?.value = Double(redParks);
			} else {
				let blueParks = data[1] as! Int;
				self.blueParksLabel?.text = "# Climbs: \(blueParks)";
